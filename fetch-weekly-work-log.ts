...
Write the brag book entry as markdown. Output ONLY the markdown content, no explanations.`;
  ...
  if (memoryStartIdx !== -1 && memoryEndIdx !== -1) {
    ...
    // Parse items to add (look for table rows after "Items to Add to Memory")
    const addMatch = memorySection.match(/## Items to Add to Memory[\s\S]*?\|[-\s|]+\|([\s\S]*?)(?=##|$)/);
    if (addMatch) {
      const tableRows = addMatch[1].trim().split("\n").filter(row => row.startsWith("|"));
      itemsToAdd = tableRows.map(row => row.trim());
    }

    // Parse items to remove (look for list items after "Items to Remove from Memory")
    const removeMatch = memorySection.match(/## Items to Remove from Memory[\s\S]*?\n([\s\S]*?)$/);
    if (removeMatch) {
  ...
  if (contextStartIdx !== -1 && contextEndIdx !== -1) {
    ...
    const impactMatch = contextSection.match(/## Impact Log Update[\s\S]*?\|[-\s|]+\|([\s\S]*?)(?=##|$)/);
    if (impactMatch) {
    ...
    const workContextMatch = contextSection.match(/## Work Context Updates[\s\S]*?\|[-\s|]+\|([\s\S]*?)(?=##|$)/);
    if (workContextMatch) {
  ...
  if (focusStartIdx !== -1 && focusEndIdx !== -1) {
    ...
    const statusMatch = focusSection.match(/## Focus Items Status[\s\S]*?\|[-\s|]+\|([\s\S]*?)(?=##|$)/);
    if (statusMatch) {
...
async function fetchDataForWeek(
  ...
  return { issues, pages, prs, reviews };
}
...
async function main() {
  try {
    ...
    for (let i = 0; i < weeksToGenerate.length; i++) {
      ...
      const { issues, pages, prs, reviews } = await fetchDataForWeek(accountId, githubUsername, weekInfo, config);
      ...
      const markdown = generateMarkdown(issues, pages, prs, reviews, weekInfo, additionalContext, config);
