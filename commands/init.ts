...
import { resolveOpenAIApiKey } from "../lib/openai-auth";
...
function checkAIKey(provider: "anthropic" | "openai"): {
  ok: boolean;
  source?: "env" | "codex";
  reason?: string;
} {
  if (provider === "anthropic") {
    return { ok: !!process.env.ANTHROPIC_API_KEY, source: "env" };
  }
...
export async function promptAI(
    ...
    options: [
      {
        ...
        hint: "uses @anthropic-ai/claude-agent-sdk",
      },
      {
        value: "openai" as const,
        label: "OpenAI",
        hint: "uses @openai/agents SDK",
      },
    ],
  ...
  const p_ = provider as "anthropic" | "openai";
  const keyStatus = checkAIKey(p_);
  if (!keyStatus.ok) {
    if (p_ === "anthropic") {
      p.log.warn(
        `ANTHROPIC_API_KEY not found in environment.\nSet it in your shell profile: export ANTHROPIC_API_KEY="sk-..."\nGet one at: https://console.anthropic.com/keys`
      );
    } else {
      p.log.warn(
        [
          "OpenAI key not found.",
          "Checked: OPENAI_API_KEY and ~/.codex/auth.json (OPENAI_API_KEY).",
          "A ChatGPT subscription alone does not grant API access for @openai/agents.",
          'Set it in your shell profile: export OPENAI_API_KEY="sk-..."',
          "Get one at: https://platform.openai.com/api-keys",
          keyStatus.reason ? `Details: ${keyStatus.reason}` : "",
        ]
          .filter(Boolean)
          .join("\n")
      );
    }
  } else if (p_ === "openai" && keyStatus.source === "codex") {
    p.log.success("OPENAI_API_KEY found via ~/.codex/auth.json");
  } else {
    const envVar = p_ === "anthropic" ? "ANTHROPIC_API_KEY" : "OPENAI_API_KEY";
    p.log.success(`${envVar} found in environment`);
  }

  const model = await p.text({
    message: "Model override (leave blank for provider default):",
    placeholder: p_ === "anthropic" ? "claude-sonnet-4-20250514" : "gpt-5",
    initialValue: initial?.model ?? "",
  });
  cancelGuard(model);
  const modelStr = (model as string).trim() || undefined;

  return { provider: p_, model: modelStr };
}
...
export async function runInit(options?: { dryRun?: boolean }): Promise<void> {
  ...
  if (existing && !dryRun) {
    const overwrite = await p.confirm({
      message:
        "Existing configuration found. Overwrite? (current config will be backed up)",
      initialValue: false,
    });
  ...
  const ai = await promptAI();

  // Check if AI is available for document parsing
  const hasAIKey = checkAIKey(ai.provider).ok;
  ...
  const githubOrgs = await promptGitHub(legacy?.githubOrgs);

  // --- Steps 5-8: Document parsing ---
  ...
  // Build pre-filled initial values from document extractions
  const profileInitial: Partial<WorklogConfig["profile"]> = {};
  if (resumeData) {
  ...
  const careerInitial: Partial<WorklogConfig["career"]> = {};
  ...
  const coaching = await promptCoaching();
